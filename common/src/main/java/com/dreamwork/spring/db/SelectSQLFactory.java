...
public class SelectSQLFactory extends AbstractSQLFactory {
	...
	public SQLEntry buildSQLConditionWithOperator(List<SQLCondition> list)  {
        ...
        if(list != null) {
            for(SQLCondition condition:list) {
                buf.append("  and ").append(SQLUtils.convertStrToDBFormat(condition.getKey()));
                Object value =  condition.getValue();
                if(condition.getOperator() == null) condition.setOperator(SQLOperator.equal);
                switch (condition.getOperator()) {
                    ...
                    case in:
                        ...
                        String subSql = SQLOperator.inToString(values);
                        buf.append(subSql);
	 ...
	 * @param clazz 待查询的实体
	 * @param list 查询条件list({@link com.dreamwork.spring.db.pojos.SQLCondition})
	 * @param order 排序方式
	 * @param <T>
	 * @return
	 */
